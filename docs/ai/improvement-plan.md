...
**Current Gaps** ⚠️
- `uv run pytest` reports **5 failing tests (168 passed / 173 total)**, concentrated in OAuth login regressions that assume pre-seeded databases.
...
- SQLite bootstrap in ad-hoc scripts (for example `test_debug_oauth.py`) still assumes pre-created tables, leading to `OperationalError: no such table: users` on fresh runs.
- Deprecation warnings (`json_encoders`, `datetime.utcnow()`) are pervasive and must be addressed before upstream removals.
...
- [ ] Replace deprecated `datetime.utcnow()` usage with timezone-aware alternatives and modern Pydantic serializers.
...
- **Deprecation Remediation**: Adopt timezone-aware timestamps and migrate away from Pydantic `json_encoders` ahead of v3.
...
2. Resolve Pydantic and datetime deprecation warnings.
3. Update CI guidance with Ruff + pytest commands and environment setup steps.
