...
class TestUserServiceIntegration:
    ...
    async def test_database_constraint_violations(
        self, async_db_session: AsyncSession, sample_user_in_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_real_password_authentication(
        self, async_db_session: AsyncSession, sample_user_in_db
    ):
        ...
        authenticated_user = await service.authenticate_user(
            sample_user_in_db.username, "TestPass123!"
        )
        ...
        with pytest.raises(AuthenticationError, match="Invalid credentials"):
            await service.authenticate_user(
                sample_user_in_db.username, "WrongPassword123!"
            )
        ...
        authenticated_user = await service.authenticate_user(
            sample_user_in_db.email, "TestPass123!"
        )
        assert authenticated_user.id == sample_user_in_db.id

    @pytest.mark.asyncio
    async def test_user_update_with_database_constraints(
    ...
    @pytest.mark.asyncio
    async def test_password_update_with_real_hashing(
        self, async_db_session: AsyncSession, sample_user_in_db
    ):
        ...
        updated_user = await service.update_password(
            sample_user_in_db.id, password_data
        )
        ...
        authenticated_user = await service.authenticate_user(
            sample_user_in_db.username, "NewTestPass456!"
        )
        assert authenticated_user.id == sample_user_in_db.id
    ...
    @pytest.mark.asyncio
    async def test_user_deletion_with_database_operations(
        self, async_db_session: AsyncSession
    ):
    ...
    @pytest.mark.asyncio
    async def test_pagination_with_real_database_data(
        self, async_db_session: AsyncSession
    ):
        ...
        assert (
            result.total >= 15
        )  # At least our 15 users (may have others from fixtures)
    ...
    @pytest.mark.asyncio
    async def test_search_functionality_with_real_queries(
        self, async_db_session: AsyncSession
    ):
        ...
        found_users = [
            user
            for user in result.items
            if "alice" in user.username.lower() or "Alice" in user.full_name
        ]
        ...
        found_users = [
            user
            for user in result.items
            if "smith" in user.username.lower() or "Smith" in user.full_name
        ]
        assert len(found_users) >= 1

    @pytest.mark.asyncio
    async def test_concurrent_operations_race_conditions(
        self, async_db_session: AsyncSession
    ):
        ...
        for failure in failures:
            ...
            assert any(
                keyword in str(failure) for keyword in {"already", "transaction"}
            )

    @pytest.mark.asyncio
    async def test_date_range_queries_with_real_dates(
        self, async_db_session: AsyncSession
    ):
